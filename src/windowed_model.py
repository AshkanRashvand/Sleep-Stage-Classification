...
class EEGSleepDataset(Dataset):
    def __init__(self,X,Y, window_size=15, step_size=15):
        """
        ...
        # Create windows
        self.x_data, self.y_data = self._create_windows(X, Y, window_size, step_size)
        self.len = len(self.x_data)

        # Convert to torch tensors
        self.X = torch.tensor(self.x_data, dtype=torch.float32)  # (N, 3000, 1)
        self.Y = torch.tensor(self.y_data, dtype=torch.long)
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        pt = torch.clamp(pt, min=1e-8, max=1 - 1e-8)  # Avoid small/large probabilities
...
if __name__ == "__main__":
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("loss_plot_MRCNN.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("accuracy_plot_MRCNN.png")
    plt.close()
    ...
    plt.savefig("confusion_matrix_MRCNN.png")
    plt.close()
